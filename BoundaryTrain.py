...
def run(args):
    ...
    for i in range(args.epochs):
        ...
        print('epoch: {:0>4}, tra_loss: {:.4f}, val_loss: {:.4f}, tra_dice: {:.4f}, val_dice: {:.4f}, val_hausdorff: {:.4f}, time: {}min, {}sec'.\
              format(i+1, tra_loss.mean().cpu().numpy(), val_loss.mean().cpu().numpy(),tra_dice.mean().cpu().numpy(),\
                     val_dice.mean().cpu().numpy(), val_hausdorff.mean().cpu().numpy(), timedelta//60, timedelta%60))
...
def get_args():
    ...
    parser.add_argument('--slice_threshold', type=float, default=0.85)
